...
class TrelloClient(object):
    ...
    """
    def __init__(self, member, key, token,
                 type_mapping=None,
                 flagged_mapping=None):
        ...
        self.trello = TrelloApi(self.key,
                                token=self.token)
    ...
    def issues_from_board_actions(self, board):
        ...
        actions = []
        limit = 1000
        filter = ['createCard', 'updateCard', 'moveCardToBoard', 'moveCardFromBoard', 'copyCard', 'copyCommentCard']
        before = None
        ...
        while (before is None) or (before > self.from_date.date()):
            while True:
                try:
                    batch = self.trello.boards.get_action(board['id'], limit=limit, filter=filter, before=before)
                    break
                except requests.exceptions.HTTPError as exception:
                    logger.error(f"get action from trello failed with:{exception}")
        ...
        for index, action in enumerate(actions):
            ...
            work_item = next((work_item for work_item in _work_items if work_item.id == card_id), None)
                ...
                while True:
                    try:
                        ...
                        if exception.response.status_code == 404:
                            ...
                            card = None
                            break
                        logger.error(f"get cards from trello failed with:{exception}")

                if card is not None:
                    date_created = datetime.fromtimestamp(int(card['id'][0:8], 16)).strftime("%m/%d/%Y, %H:%M:%S")
                    while True:
                        try:
                            card_list = self.trello.lists.get(card['idList'])
                            break
                        except requests.exceptions.HTTPError as exception:
                            logger.error(f"get lists from trello failed with:{exception}")
                    ...
                    for label in card['labels']:
                        ...
                        if self.type_mapping != None:
                            for mapping in self.type_mapping:
                                if clean_label in self.type_mapping[mapping]:
                                    issuetype = mapping
                            labels.append(clean_label)

                    work_item = JiraLikeIssue(key=card_id,
                                              url=card['url'],
                                              fields=JiraLikeFields(labels=labels,
                                                                    summary=card['name'],
    ...
    def state_transition(self, action):
        ...
        state_transition = JiraLikeHistory(action['date'],
                                           JiraLikeHistoryItem(field="status",
