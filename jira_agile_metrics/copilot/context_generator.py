...
class AIContextGenerator(Calculator):
    ...
    def write(self):
        ...
        if self.settings["copilot_context"]:
            output_file = self.settings["copilot_context"]
            logger.info("Writing AI context data to %s", output_file)
            with open(output_file, "w") as f:
                json.dump(data, f, indent=2, default=str)
        else:
            logger.debug("No output file specified for AI context data")
    ...
    def _analyze_ageing_wip(self) -> Dict:
        ...
        ageing_data = self.get_result(AgeingWIPChartCalculator)

        if ageing_data is None or ageing_data.empty:
            return {
                "status": "no_wip_items",
                "message": "No work in progress items found",
            }
    ...
    def _analyze_throughput_trends(self) -> Dict:
        ...
        throughput_data = self.get_result(ThroughputCalculator)
    ...
    def _analyze_wip_stability(self) -> Dict:
        ...
        wip_data = self.get_result(WIPChartCalculator)
        ...
        wip_trend = (
            "increasing"
            if current_wip > avg_wip * 1.1
            else "decreasing" if current_wip < avg_wip * 0.9 else "stable"
        )
    ...
    def _analyze_cycle_time_patterns(self, cycle_data) -> Dict:
        ...
        if completed_items.empty:
            return {
                "status": "no_completed_items",
                "message": "No completed items for pattern analysis",
            }
    ...
    def _identify_actionable_items(self, cycle_data) -> List[Dict]:
        ...
        # Get WIP items
        wip_items = cycle_data[pd.isna(cycle_data[done_column])].copy()
