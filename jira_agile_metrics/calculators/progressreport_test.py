...
verbs = ["build", "launch", "integrate", "optimize", "enable", "facilitate", "MVP for", "beta-test", "alpha-test"]
nouns = [
    "stock",
    "website",
    "system",
    ...
    return extend_dict(
        custom_settings,
        {
            ...
            "progress_report_outcomes": [
                {"key": "O1", "name": "Outcome one", "deadline": None, "epic_query": None},
                {
                    "key": None,
                    "name": "Outcome two",
                    "deadline": None,
            ...
            issues=[
                ...
                Issue(
                    ...
                    changes=[
                        ...
                        Change(
                            "2018-01-04 10:01:01", [("Flagged", None, "Impediment")]
                ...
                Issue(
                    ...
                    resolutiondate=None,
                    created="2018-01-02 01:01:01",
                    changes=[Change("2018-01-03 01:01:01", [("status", "Backlog", "Next")])],
                ),
                Issue(
                    "F-3",
                    summary="Completed",
...
def test_forecast_to_complete_wip_1():
    ...
    team = Team(name="Team 1", wip=1, sampler=throughput_range_sampler(2, 2))  # makes tests predictable
...
def test_forecast_to_complete_wip_2():
    ...
    team = Team(name="Team 1", wip=2, sampler=throughput_range_sampler(4, 4))  # makes tests predictable
...
def test_forecast_to_complete_with_randomness():
    ...
    team = Team(name="Team 1", wip=2, sampler=throughput_range_sampler(4, 9))  # makes tests predictable
...
def test_calculator_no_outcomes(query_manager, settings, results):
    settings = extend_dict(
        settings,
        {"progress_report_epic_query_template": 'issuetype=epic AND Outcome="O1', "progress_report_outcomes": []},
    )
...
def test_with_large_dataset(fields, settings, results):
    ...
    settings = extend_dict(
        settings,
        {
            ...
            "progress_report_outcomes": [
                ...
                {"key": "O2", "name": "Asia launch", "deadline": None, "epic_query": None},
                {"key": "O3", "name": "Europe revamp", "deadline": None, "epic_query": None},
            ],
        },
    )
    ...
    epics = [
        Issue(
            ...
            customfield_202="%s 00:00:00" % random_date_future(today + timedelta(days=55), 65).isoformat()
    ...
    def make_story(i):
        ...
        return Issue(
            ...
            changes=[Change("%s 00:00:00" % c["date"], [("status", c["from"], c["to"])]) for c in changes[1:]],
        )
