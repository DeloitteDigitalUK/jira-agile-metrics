    ...
    return JIRA(
        ...
        issues=[
            ...
            Issue(
                ...
                resolutiondate=None,
                created="2018-01-01 08:15:00",
                changes=[Change("2018-01-02 08:15:00", [("status", "Backlog", "Build")])],  # skipping column Committed
            ),
            Issue(
                "A-15",
                ...
                changes=[
                    Change(
                        "2018-01-02 08:15:00", [("status", "Backlog", "Done"), ("resolution", None, "Withdrawn")]
            ...
            Issue(
                ...
                resolutiondate=None,
                created="2018-01-01 08:15:00",
                changes=[Change("2018-01-03 08:15:00", [("status", "Backlog", "Build")])],  # skipping column Committed
            ),
        ],
    )
...
def test_empty(query_manager, settings, minimal_cycle_time_columns, today):
    ...
    assert list(data.columns) == ["key", "summary", "status", "age", "Committed", "Build", "Test"]
...
def test_calculate_ageing_wip_with_different_done_column(query_manager, settings, results, today):
    settings.update({"done_column": "Test"})
