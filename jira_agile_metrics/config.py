    ...
    return [
        {
            ...
            "throughput_samples_window": force_int(
                "throughput_samples_window", val[expand_key("throughput_samples_window")]
            )
...
def config_to_options(data, cwd=None, extended=False):
    ...
    if "queries" in config:
        ...
        options["settings"]["queries"] = [
            {"value": q.get("value", None), "jql": q.get("jql", None)} for q in config["queries"]["criteria"]
        ]

    if "query" in config:
        options["settings"]["queries"] = [{"value": None, "jql": config["query"]}]
    ...
    if "workflow" in config:
        ...
        if not (column_names.index(options["settings"]["backlog_column"]) + 1) == column_names.index(
            ...
            raise ConfigError(
                ...
                % (options["settings"]["backlog_column"], options["settings"]["committed_column"])
            )
        ...
        if not column_names.index(options["settings"]["committed_column"]) < column_names.index(
            ...
            raise ConfigError(
                ...
                % (options["settings"]["committed_column"], options["settings"]["done_column"], column_names)
            )
