...
The AI Copilot provides two levels of analysis:

**Enhanced Pattern Detection** (default): Uses advanced statistical analysis combined with AI insights to identify:
- Cycle time patterns, trends, and anomalies
- Work-in-progress (WIP) levels, variations, and optimal ranges
- Throughput consistency, predictability, and seasonal patterns
- Workflow bottlenecks, queue buildup, and flow efficiency issues
- Outliers, exceptional cases, and their root causes
- Performance correlations and leading indicators

**Basic Analysis**: Provides fundamental statistical summaries and trend identification without deep pattern analysis.

The system uses a hybrid approach combining local statistical analysis with cloud-based AI to generate actionable insights and specific recommendations for process improvements.
...
**Important - Data Privacy**: The amount of data sent to the LLM depends on your analysis depth setting:

- **Basic Analysis** (`Analysis Depth: basic`): Only statistical summaries (means, percentiles, counts) are sent - no individual ticket data
- **Enhanced Analysis** (`Analysis Depth: enhanced`): Detailed time-series data including cycle times, throughput patterns, and WIP data is sent. This may include ticket keys and timestamps but not ticket content
- **Experimental Analysis**: Similar to enhanced with additional correlation analysis

**Your raw JIRA export is never sent**, but in enhanced/experimental modes, processed calculator results (cycle times, dates, counts) are included for pattern detection. The AI context file may reference specific ticket keys for illustration and can include selected ticket titles.

**Security recommendation**: Use `--dry-run` mode with `--generate-insights` to review exactly what data will be sent before committing to using it, especially in enhanced mode.

### How to use it

Using the AI Copilot is a two-step process:

**Step 1: Generate metrics and collect data** - Run your normal metrics generation with copilot configured in the config file (see below):

    $ jira-agile-metrics config.yml

This will generate all your usual charts and data files, plus an AI context file (e.g., `ai-context.json`) containing imperative analysis results and raw calculator data. **No LLM calls are made during this step** - it only collects and stores data for later AI analysis.

**Step 2: Generate AI insights** - Generate the insights using the `--generate-insights` flag:

    $ jira-agile-metrics config.yml --generate-insights

This will read the AI context file, perform AI pattern detection, send the structured data to your chosen LLM provider, and save the generated insights to an output file. **All LLM calls happen only during this step.** You can use the `--copilot-context` command line option to provide a specific context file. It will default to the file generated by the export step as specified in the configuration.
...
Before sending data to an external LLM service, you can preview exactly what will be sent using the `--dry-run` flag **with the `--generate-insights` command**:
...
**Note**: The `--dry-run` flag only affects the insights generation step (`--generate-insights`). When used with normal metrics generation, it has no effect since no LLM calls are made during that phase.
...
Copilot:
  ...
  Max Tokens: 2000                          # Optional: defaults to 200
  Temperature: 0.1                          # Optional: defaults to 0.1
  Analysis Depth: enhanced                   # Optional: 'basic' (default), 'enhanced', 'experimental', or 'disabled'
...
- `Azure API Version: <version>` – API version for Azure OpenAI. Defaults to
  `2024-02-15-preview`.
- `Analysis Depth: <depth>` – Level of analysis to perform. Options are:
  - `basic` (default): Fundamental summaries and trend identification
  - `enhanced`: Advanced pattern detection with statistical analysis
  - `experimental`: Cutting-edge analysis features (may be unstable)
  - `disabled`: Disable AI pattern detection entirely
